...
- plugin ZIPへPython導入器を収録し、展開後の `skills/` layoutからplan/apply/verifyを実行できることをrelease builderの統合テストで固定した。
...
- macOSで標準の `/var` alias配下に作られる一時ディレクトリを任意symlinkとして誤拒否していた問題を修正。root-ownedの既知aliasと固定された `/private/...` targetだけを例外にし、それ以外のsymlink拒否は維持した。
