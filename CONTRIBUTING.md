...
Keep the repository prompt-first and read-only by default.

- Keep `PROMPT.md` and `PROMPT.en.md` semantically aligned.
...
Before opening a pull request, inspect the Markdown files as UTF-8, check that all referenced local files exist, and review the diff for stale model, schema, privacy, and licensing claims.
