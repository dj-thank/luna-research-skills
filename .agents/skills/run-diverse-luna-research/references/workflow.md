...
5. Accept packets only after exact runtime/provenance validation; the root verifies source snapshots under the freshness rule below, deduplicates source families, and alone synthesizes conclusions.

Use [v2-runtime.md](v2-runtime.md) for native lifecycle operations and runtime capability checks. The remaining sections define this skill's parallel execution contract. Use [references/research-packet.md](research-packet.md) for the machine-readable packet and ledger fields.

## Operating hierarchy (normative)

This skill supports `flat` and recursive `hierarchical` work. Root declares `max_workflow_depth` from the decomposition, bounded by N; coordinating assignments can occur at any depth below that limit. Read [recursive delegation](recursive-delegation.md) before creating subteams. Each coordinator receives a transitive subtree grant from its parent and may allocate smaller grants to further coordinators. Terminal assignments do not spawn. Root owns the single global ledger, cell leases, source-plane/access gates, integration, shared writes, external authority and final confidence.
...
GPT-5.6 Luna with max effort is this skill's default acceptance policy and the terminal specialist policy, not a universal Codex default. The documented mixed coordinator mode is an explicit exception for coordination only. Codex custom-agent files, explicit spawn values, `[agents]` defaults, and the parent can resolve differently. Treat the live `spawn_agent` schema and the completed child rollout as runtime truth. Field names such as `fork_turns` and `fork_context` are runtime-surface details, not portable platform guarantees. Do not confuse Codex custom agents with Responses API multi-agent orchestration.
...
- the decision or question, target audience, output form, and the claims that would change the answer; infer these from the conversation before asking;
...
- Explicit model or effort values do not replace completed-rollout verification. Any terminal specialist outside Luna/max is rejected even if the request was pinned correctly. An explicitly authorized mixed coordinator uses the exact model/effort policy in [model-policy.md](model-policy.md), with the mixed CLI opt-in and real child delegation.
...
- scouts never activate invitations, redeem offers, send external messages, purchase, change accounts, publish, deploy, or write shared files. Root-authorized internal peer clarification follows [peer-evidence.md](peer-evidence.md).
...
Maintain the root ledger defined in the packet contract. Reserve one assignment before every spawn. Use followup_task for related clarification or refinement and count the activation request. Keep its operational evidence distinct from accepted initial-turn receipts; apply [v2-runtime.md](v2-runtime.md) when using the result. Descendant spawning is off by default; if the user or parent workflow explicitly permits it, every descendant consumes `N`, receives a unique cell, and requires its own runtime receipt and parent-call binding.
...
Stop new work when a hard budget or deadline is reached, or every priority cell is covered or has an explicit per-cell gap reason. Quota allocation protects perspective diversity; it is not a reason to spend all N attempts. Close unused planned rows as `not_dispatched/excluded`, with null `started_at`, a `finished_at` decision time and a reason; they do not count as started attempts or completed evidence. Report accepted quota evidence separately from explicit gaps. Resolve every row before synthesis rather than leaving optional rows `planned/pending`.

### Adaptive collection and source access

After the initial route proof, dispatch ready independent cells before waiting. Collect arrivals and start work whose dependencies are now satisfied; a numbered wave is accounting, not a requirement to wait for its slowest optional scout. Reserve verification time early enough to affect the final answer.

Each additional assignment names a material unresolved claim and the observation that could change the answer. Measure yield as a newly supported or refuted claim, a resolved contradiction, or a changed decision/confidence. Page counts, agent counts, and repeated agreement are not yield. After two verified no-yield assignments, close optional expansion. Reuse still-fresh root evidence when the input is unchanged. A mandatory gap closes as unknown when budget/access prevents resolution; it never becomes a supported claim.

Assign an owner for a shared source URL/dataset/version before concurrent retrieval. Different cells may analyze different claims from the same upstream family; use one permitted snapshot when practical and count it once as independent support. The owner routes access problems to root so another scout does not unknowingly repeat the same failing request.

For an explicit access denial, authentication barrier, CAPTCHA, or rate limit, stop automatic retries to that source and report the canonical URL, retrieval time, tool/status, and affected claim. A generic retrieval failure is an access gap, not proof that the page is absent, false, safe, or the cause of a user's browser block. Root selects an accessible primary source or records the gap; a transient retry needs a changed condition or an explicit retry window, an owner, and remaining budget. Do not expand to alternate accounts, identities, or provider mutations to rescue a research cell. Public authorized alternatives remain useful independent work.

If a correction arrives mid-run, root steers or interrupts affected owners and retains unaffected evidence. Record an already-started superseded attempt as `abandoned/excluded` with `cancel_reason`, or a completed superseded result as `completed/rejected`; close an unstarted one as `not_dispatched/excluded`. Superseded is a reason, not an additional execution state. A continuation may clarify an existing candidate; its acceptance follows the separate activation-evidence rule in v2-runtime.md. Count started attempts against N without presenting them as current coverage.
...
Verify the sources supporting every conclusion-grade claim. Record the root-opened snapshot/locator and time; reuse it when still fresh and unchanged, and reopen on a material change. Prefer direct primary or official evidence; otherwise require independent, high-quality corroboration. Separate sourced fact, source interpretation, and inference. Preserve contradictions and lower confidence for weak locators, inaccessible primary material, stale evidence, or dependent sources. Build a compact claim-to-evidence map for the conclusions that drive the answer: claim, strongest support, strongest counterevidence, source family, applicable date/version, root verification, and supported/qualified/unresolved verdict. Do not turn every background fact into a separate artifact.

Check whether disagreements actually concern the same population, version, jurisdiction, time, or metric before treating them as contradictions. Direct authority may settle an official specification with one family; an effectiveness or comparative-performance claim needs matching methods and suitable independent evidence. A failed search establishes only that the bounded search found nothing. State its searched universe and missing evidence rather than asserting nonexistence.
...
Return an answer-first synthesis with citations beside claims, material contradictions, confidence changes, unknowns, and a next query only for a material unresolved claim. Include a method note with:
...
- Scouts write no local files, shared state, external messages, accounts, provider state, or public artifacts. Internal peer clarification preserves the original source-plane and no-mutation boundary.
