...
Count every continuation request as its own non-accepted assignment attempt in the same N/W ledger; a prose-only activation log is insufficient. For Research, use the concrete row mapping in [research-packet.md](research-packet.md).
